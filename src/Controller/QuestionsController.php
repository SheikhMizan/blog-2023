...
class QuestionsController extends AppController
{
    ...
    //     $imageData = $this->getRequest()->getData('image');
    //     if (!empty($imageData)) {  
    //         $imagePath = WWW_ROOT . 'img' . DS . 'uploads' . DS . $imageData['name'];
    //         move_uploaded_file($imageData['tmp_name'], $imagePath);
    //         $question->image = 'uploads/' . $imageData['name'];
    //     }
...
public function add()
{
    ...
    if ($this->getRequest()->is('post')) {
        ...
        $dimage     = $this->request->getUploadedFile('image');     
        if (!empty($imageData)) {
            $imagePath = WWW_ROOT . DS . '/img'. DS . $imageData['name'];
            move_uploaded_file($imageData['tmp_name'], $imagePath);
            $question->image = '/' . $imageData['name']; // Store the path in the database with a leading slash
        }
